...
fn bench_pipeline(c: &mut Criterion) {
    ...
    write_wav(&path, SAMPLE_RATE, CHANNELS, TRACK_SECONDS as usize, |frame| {
        let t = frame as f64 / f64::from(SAMPLE_RATE);
        ((t * 440.0 * std::f64::consts::TAU).sin() * 0.5 * 32767.0) as i16
    });
            ...
            b.iter_custom(|iters| {
                ...
                for _ in 0..iters {
                    ...
                    let config = PumpConfig {
                        info: track_info(&path, TRACK_SECONDS as usize, "bench", "bench track"),
                        start_position_ms: 0,
                        end_time_ms: None,
                        volume: 100,
